...
switch($level)
{
	case 1: $emails = "SELECT email FROM users WHERE year=".$_SESSION['usr']['year']." and department='".$_SESSION['usr']['department']."';";
			break;

	case 2: $emails = "SELECT email FROM users WHERE year=".$_SESSION['usr']['year'].";";
			break;
	
	case 3: $emails = "SELECT email FROM users WHERE department='".$_SESSION['usr']['department']."';";
			break;
	
	case 4: $emails = "SELECT email FROM users WHERE year=".$_SESSION['usr']['year']." or department='".$_SESSION['usr']['department']."';";
			break;
			
	case 5: ;		
	case 6: $emails = "SELECT email FROM users";
}
...
{
	mail($row_emails['email'],$subject,$message,$headers);
}
