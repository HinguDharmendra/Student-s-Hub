	...
	<?php include('common.php');?>
		<div id="content">
			...
			if(empty($topic) || empty($detail))
			{
				...
				header('Refresh:3; URL=create_topic_fes.php');
				exit();
			}
			$name=$_SESSION['name'];
			$email=$_SESSION['email'];
			$datetime=date("d/m/y"); //create date time
			$sql="INSERT INTO forum_question_fes (topic, detail, name, email, datetime)VALUES('".$topic."','".$detail."','".$name."','".$email."','".$datetime."')";
			$result=mysql_query($sql);
			if($result)
			{
				echo "Successful<BR>";
				header('Refresh:3; URL=main_forum_fes.php');
			}
			else 
			{
				echo "ERROR";
